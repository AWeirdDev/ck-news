...
class MarkdownTransformer:
    """Transforms parsed elements into Markdown text."""

    __slots__ = ("elements", "parts")
    ...
    def transform_inner(
        ...
        for element in elements:
            anchor = element.as_anchor()
            if anchor:
                inner_text = _inner_as_plain_text(anchor.inner)
                self.add(f"[{inner_text}]({anchor.href})")
                continue

            button = element.as_button()
            if button:
                inner_text = _inner_as_plain_text(button.inner)
                self.add(f"[{inner_text}] (button id={button.id})")
                continue

            checkbox = element.as_checkbox_input()
            if checkbox:
                mark = "x" if checkbox.checked else " "
                self.add(f"- [{mark}] (checkbox id={checkbox.id})")
                continue

            radio = element.as_radio_input()
            if radio:
                mark = "x" if radio.checked else " "
                self.add(f"- [{mark}] (radio id={radio.id})")
                continue

            text_input = element.as_text_input()
            if text_input:
                placeholder = text_input.placeholder or ""
                self.add(
                    f'[______] (text input id={text_input.id}, placeholder: "{placeholder}")'
                )
                continue

            url_input = element.as_url_input()
            if url_input:
                placeholder = url_input.placeholder or ""
                self.add(
                    f'[______] (url input id={url_input.id}, placeholder: "{placeholder}")'
                )
                continue

            heading = element.as_heading()
            if heading:
                inner_text = _inner_as_plain_text(heading.inner)
                self.add(f"{'#' * heading.level} {inner_text}")
                continue

            paragraph = element.as_paragraph()
            if paragraph:
                sub = MarkdownTransformer(paragraph.inner)
                sub.transform_inner(paragraph.inner)
                self.add("\n" + " ".join(sub.parts) + "\n")
                continue
            ...
            if image:
                alt = image.alt or ""
                self.add(f"![{alt}]({image.src})")
                continue

            label = element.as_label()
            if label:
                inner_text = _inner_as_plain_text(label.inner)
                self.add(f"**{inner_text}**")
                continue

            select = element.as_select()
            if select:
                self.add("(select)")
                self.transform_inner(select.inner)
                continue

            option = element.as_option()
            if option:
                self.add(f"- {option.text}")
                continue

            details = element.as_details()
            if details:
                self.transform_inner(details.inner)
                continue

            summary = element.as_summary()
            if summary:
                inner_text = _inner_as_plain_text(summary.inner)
                self.add(f"**{inner_text}**")
                continue
    ...
    def transform(self) -> None:
        """Transform all elements. Call before `text()`, or just call `text()` directly."""
        self.transform_inner(self.elements)

    def text(self) -> str:
        """Return the Markdown string, transforming first if needed."""
        if not self.parts:
            self.transform()
        return "\n".join(self.parts)


def _inner_as_plain_text(elements: List[Element]) -> str:
    """Recursively extract plain text from a list of elements."""
    parts: List[str] = []
    for element in elements:
        text = element.as_text()
        if text:
        ...
        anchor = element.as_anchor()
        if anchor:
            parts.append(_inner_as_plain_text(anchor.inner))
            continue
        heading = element.as_heading()
        if heading:
            parts.append(_inner_as_plain_text(heading.inner))
            continue
        paragraph = element.as_paragraph()
        if paragraph:
            parts.append(_inner_as_plain_text(paragraph.inner))
            continue
        label = element.as_label()
        if label:
            parts.append(_inner_as_plain_text(label.inner))
            continue
        summary = element.as_summary()
        if summary:
            parts.append(_inner_as_plain_text(summary.inner))
            continue
        button = element.as_button()
        if button:
            parts.append(_inner_as_plain_text(button.inner))
            continue
    return "".join(parts)
